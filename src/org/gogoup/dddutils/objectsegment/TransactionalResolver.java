...
public abstract class TransactionalResolver extends TransientParameterResolver {
	...
	/*
	 * to count the number of how many times of "PREPARE_TRANSATION" for each of transactions.
	 */
	private Map<String, Integer> preparedTransactionCounters; 
	private Set<String> rollbackTransactionIds;
	
	public TransactionalResolver(String[] names) {
		super(null);
		
		//reassemble the new names array.
		this.names = new String[names.length + 4];		
		this.names[0] = TransactionalResolver.PREPARE_TRANSACTION;
		this.names[1] = TransactionalResolver.COMMIT_TRANSACTION;
		this.names[2] = TransactionalResolver.ROLLBACK_TRANSACTION;		
		this.names[3] = TransactionalResolver.IS_COMMITTABLE;
	...
	public boolean setParameter(ParameterPool pool, String name, Object value) {
		...
		if(TransactionalResolver.PREPARE_TRANSACTION.equals(name))
		{
			if(null == value
					|| TransactionalResolver.APP_TRANSACTION.equals(value))
			{				
				...
				//+1
				increaseCounter(TransactionalResolver.APP_TRANSACTION, this.preparedTransactionCounters);
			...
			//start counting for the specified transaction if there is no app transaction prepared
			if(!this.isAppTransactionPrepared)
			{
				//+1
		...
		if(TransactionalResolver.COMMIT_TRANSACTION.equals(name))
		{
			//for app transaction
			if(null == value
					|| TransactionalResolver.APP_TRANSACTION.equals(value))
			{				
				...
				//-1
				decreaseCounter(TransactionalResolver.APP_TRANSACTION, this.preparedTransactionCounters);
				//commit all holding transaction at once the app transaction counter decrease to 0
				if(0==readCounter(TransactionalResolver.APP_TRANSACTION, this.preparedTransactionCounters))
				{
			...
			if(!this.preparedTransactions.containsKey(id)
					|| null == this.preparedTransactions.get(id))
				throw new RuntimeException("Transaction, "+id+" has not prepared yet.");
			
			//decrease the counter for the specified transaction if there is no app transaction prepared
			if(!this.isAppTransactionPrepared)
			{
				//-1
				decreaseCounter(id, this.preparedTransactionCounters);
				//commit the transaction right away as long as the counter decrease to 0.
		...
		if(TransactionalResolver.ROLLBACK_TRANSACTION.equals(name))
		{	
			//for app transaction
			if(null == value
					|| TransactionalResolver.APP_TRANSACTION.equals(value))
			{				
				...
				//-1
				decreaseCounter(TransactionalResolver.APP_TRANSACTION, this.preparedTransactionCounters);
				//rollback all holding transaction at once the app transaction counter decrease to 0
				if(0==readCounter(TransactionalResolver.APP_TRANSACTION, this.preparedTransactionCounters))
					this.rollbackAll();
	...
	public boolean isReadOnly(ParameterPool pool, String name) {
		if(TransactionalResolver.PREPARE_TRANSACTION.equals(name)) return false;
		if(TransactionalResolver.COMMIT_TRANSACTION.equals(name)) return false;
		if(TransactionalResolver.ROLLBACK_TRANSACTION.equals(name)) return false;
		return true;
	}
