...
public class DefaultAppSession implements AppSession, AppSessionContext {
    ...
    public DefaultAppSession(String sessionKey, DefaultAppSessionFactory factory) {
        this(sessionKey, new DefaultParameterPool(), factory);
    }
    
    public DefaultAppSession(String sessionKey, ParameterPool parameterPool, DefaultAppSessionFactory factory) {
        
        this.sessionKey = sessionKey;
        this.parameterPool = parameterPool;
        this.objectSegment = null;
        this.factory = factory;
        this.applicationId = this.factory.getApplicationId();
        this.reset();
    }
    
    public ObjectSegment getObjectSegment() {
        return this.objectSegment;
    }
    ...
    public boolean setParameter(String name, Object value) {
        ...
        if (result && !this.parameterPool.isTransient(name)) {
            // System.out.println("HERE======>DefaultAppSession.setParameter() name "+name+
            // "; THREAD: " +Thread.currentThread().getId());
            // System.out.println("HERE======>DefaultAppSession.setParameter() OS "+this.objectSegment+
            // "; THREAD: " +Thread.currentThread().getId());
    ...
    public String getSessionKey() {
        return this.objectSegment.getSessionKey();
    }
    ...
    public void update() throws NoSuchAppSessionException {
        ...
        if (!this.loadPersistentSP()) {
            ...
            this.objectSegment = new AppSessionOS(this.applicationId, this.sessionKey,
                    new HashMap<String, Object>(), new Date(System.currentTimeMillis()));
        ...
        AppSessionOS oldSession = this.spi.selectByKey(this.applicationId, this.sessionKey);
        // If the session has been destroyed by other thread
        if (STATUS_UPDATED == this.status
                && null == oldSession)
        ...
        if (null == oldSession) {
            ...
            this.parameterPool.restore(this.objectSegment.getStates()); // restore
                                                                        // parameters
        }
    ...
    public void sync() throws InconsistentAppSessionException, NoSuchAppSessionException {
        ...
        if (this.objectSegment.isNew() && hasSession)
            throw new InconsistentAppSessionException("Need to update session \'" + this.objectSegment.getSessionKey()
                    + "\' before sync it.");
        
        // if this session has been destroyed by other thread
        if (this.objectSegment.isDirty() && !hasSession)
            throw new NoSuchAppSessionException("Session \'" + this.objectSegment.getSessionKey()
                    + "\' has been deleted (Invoking this method again will recreate this session).");
        
        // if this session has been destroyed.
        if (this.objectSegment.isDeleted() && !hasSession)
            return;
    ...
    public void close() {
        ...
        this.checkForNewStatus(STATUS_RESET);
        // if(!this.objectSegment.isCleanMark())
        // TODO: log message for warning there are some changes not been saved
        // yet.
        this.reset();
    ...
    public void destory() throws InconsistentAppSessionException, NoSuchAppSessionException {
        ...
        this.sync(); // delete session's states via persistence service
                     // provider.
        this.factory.removeSession(this.getSessionKey()); // remove all sessions
                                                          // key from the pool
    ...
    public Object getApplicationParameter(String name) {
        return factory.getApplicationParameter(name);
    }

    @Override
    public void setApplicationParameter(String name, Object value) {
        factory.setApplicationParameter(name, value);
    }

    @Override
    public long registerSessionParameterResolver(ParameterResolver resolver) {
        return this.parameterPool.registerParameterResolver(resolver);
    }

    @Override
    public void deregisterSessionParameterResolver(long id) {
        this.parameterPool.deregisterParameterResolver(id);
    }

    private AppSessionFactory getFactory() {
        return this.factory;
    }
    ...
    private boolean loadPersistentSP() {
        ...
        if (null == this.spi) {
            ...
            if (null == this.spi) {
                // TODO: display warning level log message
                // "Failed to load persistency layer SPI. This will cause session states cannot be propagated."
                return false;
            }
        }
    ...
    private void reset() {
        ...
        if (null != this.objectSegment)
            this.objectSegment.clean();
    ...
    private void checkForNewStatus(int newStatus) {
        ...
        if (STATUS_DESTORYED == this.status) {
            if (STATUS_RESET == newStatus || STATUS_UPDATED == newStatus || STATUS_DESTORYED == newStatus)
                throw new IllegalStateException();
        }
    }
}
