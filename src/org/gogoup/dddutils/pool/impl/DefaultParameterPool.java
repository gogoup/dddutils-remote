...
public class DefaultParameterPool implements ParameterPool{
	...
	public Object getParameter(String name) {
		if(!isParameterNameRegistered(name)) return null;
			...
			if(null != dependentParameterNames)
			{
				for(String dname: dependentParameterNames)
				{
					if(dname.equals(pname))
						throw new IllegalArgumentException("Parameter name \""+pname+"\" is depending on self (This would cause a dead dependent loop).");
					
					if(!this.isParameterNameRegistered(dname))
						throw new RuntimeException("Parameter name \""+pname+"\" failed to verify dependent parameter, "+dname);
	...
	public void restore(Map<String, Object> states) {
		...
		//only restore the parameters registered is keep the maximum backward compatibilty
		String[] names = this.getParameterNames();
		for(String name: names)
		{
