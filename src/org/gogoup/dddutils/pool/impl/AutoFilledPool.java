 ...
 * 
 * 
 * AutoFilledPool |------------------------------| | | | | | 0 min size capacity
 * max
 * 
 * 
 */
...
public abstract class AutoFilledPool<T> implements Pool<T> {
    ...
    private int minCapacity; // the minimum number of alive channels in the
                             // pool.
    private int maxCapacity;
    private int increment;
    private int capacity; // current capacity of pool (need to be greater equal
                          // than minimum capacity and less equal than maximum
                          // capacity)
    private List<T> pool;
    ...
    public AutoFilledPool(int minCapacity, int maxCapacity, int increment, PoolDelegate<T> delegate) {
        ...
        if (increment > maxCapacity)
            throw new IllegalArgumentException("Increment cannot exceed max capacity, (" + increment + " > "
                    + maxCapacity + ").");
        if (minCapacity > maxCapacity)
            throw new IllegalArgumentException("Min capacity cannot exceed max capacity, (" + minCapacity + " > "
                    + maxCapacity + ").");
    ...
    public AutoFilledPool(int initCapacity, int minCapacity, int maxCapacity, int increment, long timeout,
            PoolDelegate<T> delegate) throws PoolTimeoutException {
        this(minCapacity, maxCapacity, increment, delegate);
        
        if (initCapacity > maxCapacity)
            throw new IllegalArgumentException("Initial capacity cannot exceed max capacity, (" + initCapacity + " > "
                    + maxCapacity + ").");
        if (initCapacity < minCapacity)
            throw new IllegalArgumentException("Initial capacity cannot be less than min capacity, (" + initCapacity
                    + " < " + minCapacity + ").");
            ...
            try {
                while (true) {
                    ...
                    if (null != item) {
                        ...
                        if (null == this.delegate || this.delegate.willBorrow(this, item)) {
                        ...
                        this.capacity--;// shrink pool by one
                        item = null;
                        continue;
                    }
    ...
    public void returnItem(T item) {
        ...
        synchronized (this) {
            ...
            if (null == this.delegate || this.delegate.willReturn(this, item)) {                
                this.pool.add(item);
            } else {
                //System.out.println("HERE======>AutoFilledPool.returnItem() Failed To Return Item: "+item);
                this.capacity--;// shrink pool by one
            }
    ...
    /**
     * Check if the capacity of the pool reach the maximum limit.
     * 
     * @return
     */
    ...
    /**
     * Calculates the increment based on the current number of items in the pool
     * and current capacity of the pool.
     * 
     * @return
     */
    private int calculateIncrement() {
        ...
        int nextIncrement = 0;
        // check if the current number of items in the pool is lower than the
        // minimum capacity requirement.
        // and the current capacity has not reach the maximum capacity
        // limitation yet.
        if ((this.pool.size() < this.minCapacity || this.pool.size() == 0) && !this.isFull()) {
